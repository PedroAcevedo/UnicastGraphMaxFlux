function [minmaxflujo,nodos_cod,swseguir,swp] = ff_opt_minflujo4(minflujo, nodos_cod, minmaxflujo, dest,segnodtot,swseguir)
jj=1;
ndest=length(dest);
tdrmin=[];
%ll=0;
...
%                 [codseg,ijl] = ff_conscodseg2(codseg, nodos_cod, tdrmin(hkl), ijl);
                [codseg] = ff_conscodseg3(codseg, nodos_cod, tdrmin(hkl));
            else
                ikl=ikl+1;
            end
        end
...
while ~swc && hkl<=length(codseg)
    ...
    if ikl-length(tdrmin)==1 %Terminó todas las pruebas en tdrmin con un string de nodos codificación
        ...
        swm=true;
        mm=1;
        while swm && mm<=length(sumiseg)
            if nnz(contarseg(sumiseg(mm),:)) < minflujo 
                swm=false;
            else
                mm=mm+1;
            end
...
swp=false;
if swc
    [minmaxflujo, nodos_cod] = ff_actflujo(minmaxflujo, nodos_cod, tdrmin(selsum).tdrmn(selrut).camino, tcod);
else
    swseguir=false;
    segnodtot=find(minmaxflujo(1,:)==1);

    if length(segnodtot)==minflujo && ~isempty(tdrmin)  % ESTO ES UN CAMBIO PARA  LA  MONOGRAFÍA
        [tcod,selsum,selrut,swp] = ff_verdet(tdrmin,minflujo,dest,segnodtot, nodos_cod);
        if swp
            [minmaxflujo, nodos_cod] = ff_actflujo(minmaxflujo, nodos_cod,tdrmin(selsum).tdrmn(selrut).camino, tcod);    
        end
    end
