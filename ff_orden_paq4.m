...
%                end
                ...
                elseif find(temp==ab)
                   ...
                   % yy=find(temp==xy);
                    if xpqt=='x' || xpqt=='y'
                        posnd=find(rel(:,2)==pos(xx));
                        posc=rel(posnd,1);
                        posns=find(rel(:,1)==pos(xx));
                        listpaq(pos(xx))=ff_intercambio(ab);
                        letinv=ff_intercambio(listpaq(pos(xx)));
                        listpaq(posc)=letinv;
                        if ~isempty(posns)
                            posc=rel(posns,2);
                            listpaq(posc)=letinv;
                ...
                if find(temp=='c')  % Se debe  eliminar 
                    xpqt=temp(xx);
                    listpaq(posdnc)=ff_intercambio(xpqt);  %fin de se debe eliminar
                elseif find(temp==ab)
                    xpqt=temp(xx);
                   % yy=find(temp==xy);
                    if xpqt=='x' || xpqt=='y'
