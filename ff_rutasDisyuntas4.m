    ...
    while ~isempty(find(marca==0))
        ...
        for ii=2:length(rut)
            if mncol > rutas(rut(ii)).sumcol
               ...
               if mncol<=rutas(rut(ii)).sumcol
                  if sgcol>rutas(rut(ii)).sumcol || sgcol==mncol %la segunda puede ser solo cuando el menor es el primero de la lista
                     sgcol=rutas(rut(ii)).sumcol;
                     srut=ii;
                  end
               end
            end
        ...
        gemelo=intersect(rutas(rut(prut)).camino, rutas(rut(srut)).camino);
        ...
        if mn-length(gemelo)==1
            rutgem(nd).nsg(posp)=rutas(rut(prut)).camino(2);
            % TAL VEZ QUITAR
            %rutgem(nd).rpri=rut(prut);
            %rutgem(nd).rseg=rut(srut);
            % TAL VEZ QUITAR
            rutgem(nd).pri(posp).camino=rutas(rut(prut)).camino;
            rutgem(nd).seg(posp).camino=rutas(rut(srut)).camino;
            posp=posp+1;
        end      
...
% %     SEGMENTO A ELIMINAR
        ...
        if ~isempty(mincomun) %% AGREGADO PARA  PROBAR
            for jj=1:length(rutasdest(dirflrut(ii)).rutamax)
                if ~isempty(find(rutasdest(dirflrut(ii)).rutamax(jj).camino(2)==mincomun))
                    for kk=1:length(rutasdest(dirflrut(ii)).rutamax(jj).camino)-1
                        minmaxflujo(rutasdest(dirflrut(ii)).rutamax(jj).camino(kk),...
                            rutasdest(dirflrut(ii)).rutamax(jj).camino(kk+1))=1;
                    end
                    numflujo=numflujo+1;
                end
            end
        ...
        for jj=1:minflujo - numflujo
          ...
          %      lpos=find(segnod(dirflrut(ii)).lista==seg(kk));
                ...
                for ll=1:length(rutasdest(dirflrut(ii)).rutamax(lpos).camino)-1
                    minmaxflujo(rutasdest(dirflrut(ii)).rutamax(lpos).camino(ll),...
                        rutasdest(dirflrut(ii)).rutamax(lpos).camino(ll+1))=1;
                end
            %end
...
%    [smflujo, iz, inz] = ff_recorte_matriz(mflujo);
%    bg=biograph(smflujo,[],'ShowWeights','on');
%    for ii=1:length(inz)
%        bg.nodes(ii).ID=strcat('Node ',num2str(inz(ii)));
%        %bg.nodes(ii).Shape='circle';
%    end
%    view(bg)
...
% AGREGADO DE UNO EN UNO
% ff_grafo(minmaxflujo,dest,nodos_cod);
% [minmaxflujo, nodos_cod,swseguir,swp] = ff_opt_minflujo3(minflujo,nodos_cod,minmaxflujo,dest,segnodtot,swseguir);
% ff_grafo(minmaxflujo,dest,nodos_cod);
% [minmaxflujo, nodos_cod,swseguir,swp] = ff_opt_minflujo3(minflujo,nodos_cod,minmaxflujo,dest,segnodtot,swseguir);
% ff_grafo(minmaxflujo,dest,nodos_cod);
% [minmaxflujo, nodos_cod,swseguir,swp] = ff_opt_minflujo3(minflujo,nodos_cod,minmaxflujo,dest,segnodtot,swseguir);
