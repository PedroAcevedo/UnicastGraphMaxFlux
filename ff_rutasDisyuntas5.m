...
for nd=1:ndest   
    ...
    for jl=1:length(tdr)
        fprintf('%2.0f ',jl)
        for ll=1:nn
            if ~isempty(find(tdr(jl).camino==ll))
                fprintf('%s','x,');
            else
                fprintf('%s',' ,');
            end
    ...
    for jl=1:length(tdr)
        fprintf('%2.0f %2.0f ',jl,tdr(jl).camino);
        fprintf('\n');
    end
    ...
    while ~isempty(find(marca==0))
        ...
        for ii=2:length(rut)
            if mncol > tdr(rut(ii)).sumcol
               ...
               if mncol<=tdr(rut(ii)).sumcol
                  if sgcol>tdr(rut(ii)).sumcol || sgcol==mncol %la segunda puede ser solo cuando el menor es el primero de la lista
                     sgcol=tdr(rut(ii)).sumcol;
                     srut=ii;
                  end
               end
            end
        ...
        pos2=find(segnodtot==tdr(rut(prut)).camino(2));
        contsegnod(pos2)=contsegnod(pos2)+1;
        gemelo=intersect(tdr(rut(prut)).camino, tdr(rut(srut)).camino);
        ...
        if mn-length(gemelo)==1
            rutgem(nd).nsg(posp)=tdr(rut(prut)).camino(2);
            %%%% TAL VEZ QUITAR
            %rutgem(nd).rpri=rut(prut);
            %rutgem(nd).rseg=rut(srut);
            %%%% TAL VEZ QUITAR
            rutgem(nd).pri(posp).camino=tdr(rut(prut)).camino;
            rutgem(nd).seg(posp).camino=tdr(rut(srut)).camino;
            posp=posp+1;
        end      
...
[minmaxflujo, nodos_cod] = ff_opt_minflujo(minflujo,nodos_cod,minmaxflujo,dest);
