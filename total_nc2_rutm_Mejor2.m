...
kk=1;
for i=1:nn
    if length(find(fa_min(:,i)~=0))>1 && isempty(des.nodo(des.nodo==i))  
...
%     for j=1:length(nodos_cod)
%         nct(j)=nodos_cod(j).nodo;
%     end
%     set(nc.Nodes(nct(1,:)),'Color',[0 1 0]);
% end
%Fin impresión de grafo general de flujo máximo sin correcciones.
...
%            ncod_com=intersect(find(fa_min(:,nodos_cod(in).nodo)==1),...
%                find(fa_min(:,nodos_cod(jn).nodo)==1));
%            if ~isempty(ncod_com)
%                fa_min(ncod_com(1),nodos_cod(jn).nodo)=0;
%            end
%        end
%    end
...
%        if length(find(fa_min(:,nodos_cod(i).nodo)==1))<=1
